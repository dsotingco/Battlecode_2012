...
public class SoldierControl extends InteractionControl
{
	//additional fields
	
	//constructor
	public SoldierControl(RobotController myRC, MapLocation pl, Direction pd, boolean ib, BugDirection bd, MapLocation bs, int bc)
	{
	...
	public void Go()
	{
		...
		sleepFluxThreshold = RobotType.SOLDIER.moveCost; 
		...
		if(myRC.getFlux()>sleepFluxThreshold)
		{
			...
			if(msgs.length>0)
			{
				Message mymsg = findMessage(msgs);
				if(mymsg==null)
				{
					defaultBehavior();
				}
				...
				{
					myRC.setIndicatorString(1,"Got message to attack " + mymsg.locations[0].toString());
					attackMove(mymsg.locations[0],1,SurroundDirection.INWARD);
				}
			}
			else
			{
				//in the absence of messages, do the following
				defaultBehavior();
			}
		}
	}
	...
	public void attack()
	{
		if(myRC.isAttackActive()==false)
		{
			...
			if(ae.length>0)
			{
				//first, look for enemy archons
				int index = 0;
				while(index<ae.length-1 && ae[index].type.equals(RobotType.ARCHON)==false)
				{
					index++;
				}
				if(index<ae.length-1)
				{
					//then attack the enemy archon
				...
				else if(ae[index].type.equals(RobotType.ARCHON)==true)
				{
					//then attack the enemy archon
					MapLocation theLocation = ae[index].location;
					RobotType theType = ae[index].type;
				...
				{
					//if you found no archons, look for scorchers
					index = 0;
